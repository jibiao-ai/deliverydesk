...
export default function DashboardPage() {
  ...
  const statCards = [
    { label: '常用网站链接', value: stats?.website_links ?? '--', icon: Globe, iconBg: 'bg-blue-100', iconColor: 'text-blue-600', trend: '已收录' },
  ...
  const quickActions = [
    { label: '常用网站', desc: '快速访问交付常用工具和系统', icon: Globe, iconBg: 'bg-blue-50', iconColor: 'text-blue-600', page: 'websites' },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          <div className="lg:col-span-2 bg-white rounded-xl border border-gray-200 shadow-sm">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <button onClick={() => setActivePage('chat')} className="flex items-center gap-1 text-sm font-medium text-blue-600">
              ...
              {recentConvs.length > 0 ? (
                  ...
                  {recentConvs.slice(0, 6).map((conv, i) => (
                    <div key={conv.id || i} className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors group"
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-gray-700 truncate group-hover:text-blue-600">{conv.title || `对话 ${i + 1}`}</p>
                      </div>
                ...
                <div className="text-center py-10">
                  ...
                  <button onClick={() => setActivePage('chat')}
                    className="text-sm font-medium px-4 py-2 rounded-lg text-white bg-blue-600">
              ...
              {quickActions.map((action, i) => {
                ...
                return (
                  <button key={i} onClick={() => setActivePage(action.page)}
                    className="w-full flex items-center gap-3 px-4 py-3 rounded-xl border border-gray-100 hover:border-blue-600 hover:bg-blue-50 transition-all text-left group">
